...
DATE = '5_reward_priority'
FILENAME = 'reward_priority.h5' #weight file name
    ...
    def sendTo_next_node(self, env, dpid):
        ...
        if dpid < 4:
            ...
            for _ in range(len(flows.items)):
                ...
                delay = delay = f.bits_/20000000.0 + f.queueing_delay_ + f.node_arrival_time_ - f.generated_time_
    ...
    def gcl_extract(self, env):  # mainprocess
        ...
        while not self.done:  # 1회의 episode가 종료될 때 까지 cycle을 반복하는 MAIN process
            ...
            for n in range(NODES):
              ...
              gcl[n + 1] = self.gcl_predict(self.state[n + 1].reshape((1, STATE_SIZE)))
            ...
            for n in range(NODES):  # convey the predicted gcl and get states of queue
                _, reward, self.done = self.step(n + 1, qlen, qdata)
