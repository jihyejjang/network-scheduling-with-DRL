...
class GateControlSimulation:
    ...
    def flow_generator(self, time, type_num, fnum):  # flow structure에 맞게 flow생성, timestamp등 남길 수 있음
        ...
        if type_num == 1:  # c&c
            ...
            f.generated_time_ = time #- self.start_time #수정
            f.queueing_delay_ = 0
    ...
    def sendTo_next_node(self, env, dpid):
        ...
        if not (len(flows.items)):
            return
        if dpid < 4:
            #print(env.now, ' {} 노드에서 {}노드로 전송(sendTo)'.format(dpid, route[dpid - 1] + 1))
    ...
    def episode(self, env):  # mainprocess
        ...
        for episode_num in range(MAX_EPISODE):
            ...
            while not self.done:  # 1회의 episode가 종료될 때 까지 cycle을 반복하는 MAIN process
                ...
                ###수정
                for n in range(NODES):
                  self.state[n + 1], _, _ = self.step(n + 1, qlen, qdata)
                  gcl[n + 1] = self.agent.choose_action(self.state[n + 1])
                  #gcl[n+1]=number_to_action(ACTION_SIZE - 1)
            ...
            delay_ = pd.DataFrame([(episode_num, np.mean(self.avg_delay[0]),
                                    np.mean(self.avg_delay[1]), np.mean(self.avg_delay[2]), np.mean(self.avg_delay[3]))],
            ...
            if ((self.total_episode >= 1000) and (self.loss_min >= np.min(loss))) or episode_num == MAX_EPISODE - 1:
                ...
                np.save("./result/" + DATE + "/" + DATE + "_S&A.npy", self.state_and_action)
            e = time.time() - s
            ...
            #추가
            reward_list.append(np.mean(rewards_all))
            et_list.append(e)
            loss_list.append(np.mean(loss))
            if episode_num%20==0:
              plt.plot(reward_list)
              plt.xlim(0,MAX_EPISODE)
              plt.savefig('./result/' + DATE + '/reward.png')
              plt.clf()
              plt.plot(et_list)
              plt.xlim(0,MAX_EPISODE)
              plt.savefig('./result/' + DATE + '/elapsed_time.png')
              plt.clf()
              plt.plot(loss_list)
              plt.xlim(0,MAX_EPISODE)
              plt.savefig('./result/' + DATE + '/loss.png')
              plt.clf()
